	...
	*/
	...
	function makeHtmlReport($data,$resolveip=false,$hosts_array=array(),$use_js=true){
		...
		foreach($users as $key=>$v){
				...
				if (isset($session_data[$con_id]) && $con_data = $session_data[$con_id] ) {
					...
					if ($was_time && $was_size) {
						...
						if ($con['bytes'] >= $was_size) {
							$curr_speed = ($con['bytes'] - $was_size) / 1024 / 1024 / $delta;
						}
					} else {
						$curr_speed = $con['bytes'] / 1024 / 1024;
					}
					
					//avg speed
					$avg_speed = $con['bytes'] / 1024 / 1024;
					if ($con['seconds'] > 0) {
			...
			if($this->use_sessions){
				$table.=sprintf("<tr><td colspan=\"3\"></td><td align=\"right\" id=\"highlight\">%01.2f MB/s</td><td align=\"right\" id=\"highlight\">%01.2f MB/s</td><td colspan=\"2\"></td>",
				$user_curr, $user_avg);
			}
		...
		$stat_row='';
		if($this->use_sessions){
			$stat_row.=sprintf("<tr class=\"total\"><td><b>Total:</b></td><td align=\"right\" colspan=\"5\"><b>%d</b> users and <b>%d</b> connections @ <b>%01.2f/%01.2f</b> MB/s (CURR/AVG)</td></tr>",
