...
class squidstat{
	...
	*/
	...
	function makeHtmlReport($data,$resolveip=false,$hosts_array=array(),$use_js=true){
		...
		$total_avg = $total_curr = $total_basic_speed = 0;
		// resort data array
		$users=array();
		switch($group_by){
			case "host":
		...
		'<th>Size</th><th>Speed</th><th>Time</th>'.
		'</tr>';
		$ausers=$acon=0;
		unset($session_data);
		...
		foreach($users as $key=>$v){
			...
			'<td style="border-left:0;" colspan="6">&nbsp;</td></tr>';
			$user_avg = $user_curr = $con_color =  0;				
			foreach ($v as $con){
				if(substr($con["uri"],0,7)=="http://" || substr($con["uri"],0,8)=="https://" || substr($con["uri"],0,6)=="ftp://"){
					...
					$uri='<a target="_blank" href="'.htmlspecialchars($con["uri"]).'">'.$uritext.'</a>';
				}
				else { if(strlen($con["uri"])>SQSTAT_SHOWLEN) $uri=htmlspecialchars(substr($con["uri"],0,SQSTAT_SHOWLEN))."...."; else $uri=htmlspecialchars($con["uri"]); }
				$acon++;
				//speed stuff
				...
				$curr_speed=0;
				$avg_speed=0;
				// Calculate basic speed for display (always available)
				if ($con['seconds'] > 0) {
					$basic_speed = $con['bytes'] / 1024 / $con['seconds'];
				} else {
					$basic_speed = 0;
				}
				...
				if($this->use_sessions){
					$table .= '<td nowrap align="right">'.( (round($curr_speed, 2) > 0) ? sprintf("%01.2f MB/s", $curr_speed) : '' ).'</td>'.
					'<td nowrap align="right">'.( (round($avg_speed, 2) > 0) ? sprintf("%01.2f MB/s", $avg_speed) : '' ). '</td>';
				}
				$table .= '<td nowrap align="right">'.$this->filesize_format($con["bytes"]).'</td>'.
				'<td nowrap align="right">'.(($basic_speed > 0) ? sprintf("%01.2f", $basic_speed * 8 / 1024) . ' Mbit/s' : '-').'</td>'.
				'<td nowrap align="right">'.$this->duration($con["seconds"],"short").'</td>'.
				'</tr>';
			}
			if($this->use_sessions){
				$table.=sprintf("<tr><td colspan=\"3\"></td><td align=\"right\" id=\"highlight\">%01.2f KB/s</td><td align=\"right\" id=\"highlight\">%01.2f MB/s</td><td colspan=\"2\"></td>",
				$user_curr, $user_avg);
			}
		...
		$stat_row='';
		if($this->use_sessions){
			$stat_row.=sprintf("<tr class=\"total\"><td><b>Total:</b></td><td align=\"right\" colspan=\"6\"><b>%d</b> users and <b>%d</b> connections @ <b>%01.2f/%01.2f</b> KB/s (CURR/AVG), Speed: <b>%01.2f</b> Mbit/s</td></tr>",
			$ausers, $acon, $total_curr, $total_avg, $total_basic_speed * 8 / 1024);
		}
		else {
			$stat_row.=sprintf("<tr class=\"total\"><td><b>Total:</b></td><td align=\"right\" colspan=\"6\"><b>%d</b> users and <b>%d</b> connections, Speed: <b>%01.2f</b> Mbit/s</td></tr>",
			$ausers, $acon, $total_basic_speed * 8 / 1024);
		}
		if($ausers==0){
			$text.='<tr><td colspan=7><b>No active connections</b></td></tr>';
		}
		else {
